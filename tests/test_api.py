...
class TestWorkingApi:
    ...
    def test_01_list_features(self):
        features = list_features(self.api)
        assert features is not None

    def test_02_list_labels(self):
        labels = list_labels(self.api)
        assert labels is not None

    def test_03_list_projects(self):
        projects = list_labels(self.api)
        assert projects is not None

    def test_04_create_features(self):
        features = [{'name': 'myFeature', 'type': 'string', 'dimension': 'scalar', 'description': 'My custom feature'}]
        create_features(self.api, 'myFeature', features)
        feature_data = describe_feature(self.api, 'myFeature')['data']
        assert feature_data == features

    def test_05_create_label(self):
        create_labels(self.api, 'myLabel', label_type='oneOf', label_one_of=['test1','test2','test3'], label_description='My description')
        label_data = describe_label(self.api, 'myLabel')
    ...
    def test_06_create_project(self):
        create_project(self.api, 'myProjectId', 'My project name', 'classification', 'myFeature', 'myLabel')
        project = describe_project(self.api, 'myProjectId')
    ...
    def test_07_predict(self):
        prediction = predict(self.api, 'myProjectId', features = { 'myFeature': 'example string' }, annotate=False, log=False)
